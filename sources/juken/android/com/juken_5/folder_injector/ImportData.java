...
public class ImportData extends ListActivity {
    ...
    private void onFileClick(Option o) {
        ...
        try {
            if (temp.equals("Prjx")) {
                String[] result = Decrypt(new BufferedReader(new InputStreamReader(new FileInputStream(new File(o.getPath(), "")))).readLine(), "Felix").split("\r\n");
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    } else if ((mulai <= 4494 || mulai > 5593) && ((mulai <= 5593 || mulai > 6874) && mulai > 6874 && mulai <= 8155)) {
                ...
                for (int i5 = 19; i5 > 0; i5--) {
                    for (int j = 0; j < 61; j++) {
                        ...
                        MappingHandle.list_pattern_current_file.add(String.format("%.2f", new Object[]{Double.valueOf(((Double.valueOf(bmTotSementara.get(posisi1 - (i5 * 61))).doubleValue() - seratus) * 100.0d) / seratus)}).replace(",", "."));
                    }
                }
        ...
        finish();
        startActivityForResult(new Intent(this, injector_timing.class), 100);
    }
